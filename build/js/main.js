...
$(document).ready(function () {
  ...
  d3.selectAll('svg').attr('width', '100%').attr('height', '1000').attr('padding', '0px 0px 0px 0px');
  ...
  $.getJSON('src/data/folderStruct.json', function (data) {
    data.forEach(function (folder, i) {
      ...
      var imagePath = $.grep(imagePaths, function (x) {
        return x.indexOf(folder.foldername) !== -1;
      });
      ...
      slide.append('button').attr('class', 'overlayToggle').text('Toggle Overlay').on('click', function () {
      ...
      image.onload = function (img) {
        // Images are defaulting to 0,0 on some browsers.
      ...
      folder.files.forEach(function (file) {
        d3.json('src/data/' + folder.foldername + '/' + file.filename + '.geojson', function (geoJSONData) {
          ...
          var path = d3.geoPath();
          overlays.selectAll('path').data(geoJSONData.features).enter().append('path').attr('d', path).attr('fill', file.color).attr('opacity', 0.7).attr('stroke', 'black').attr('stroke-width', '1');
        });
      });
    });
  });
